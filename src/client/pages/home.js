...
class Home extends Component {
    ...
    if (e.target === e.currentTarget) {
      if (e.target.id == 'off') {
        ...
        document.querySelectorAll('.toggleAmount').forEach(element => {
          ...
          request.open(
            'GET',
            'http://localhost:9080/mfp/api/adapters/AccountServiceAdapter/resource/accountBalance?userid=11'
          );
  ...
  getCIBILScore() {
    ...
    request.open(
      'GET',
      'http://localhost:9080/mfp/api/adapters/AccountServiceAdapter/resource/getCIBILScore?pan=dsds'
    );
    request.onreadystatechange = function() {
      if (this.readyState === 4 && this.status === 200) {
